...
extern 
void set_user_window_size(uint16_t width, uint16_t height)
{
	s_WinHeight = height;
	s_WinWidth = width;
}
...
void RA8875_LCD_DataWrite(uint8_t _ucRegValue)
{
    RA8875_RAM = _ucRegValue;	/* 设置寄存器地址 */
}

void RA8875_LCD_CmdWrite(uint8_t _ucRegCmd)
{
    RA8875_REG = _ucRegCmd;	/* 设置寄存器地址 */
}
uint8_t RA8875_LCD_DataRead(void)
{
    return RA8875_RAM;
}
    
void Delay2us(uint8_t Counter)
{
...
//==============================================================================
void Delay100us(uint8_t Counter)
{
	while(Counter--)
	{	Delay2us(100);
	}
...
//==============================================================================
void Delay10ms(uint8_t Counter)
{
	while(Counter--)
	{	Delay1ms(11);			
	}
...
//==============================================================================
void Delay100ms(uint8_t Counter)
{
	while(Counter--)
	{	Delay1ms(101);			
	}
...
{
    RA8875_LCD_CmdWrite(0x88);
    Delay2us(1);       
    RA8875_LCD_DataWrite(0x0c);
    Delay1ms(1);    
    RA8875_LCD_CmdWrite(0x89);
    Delay2us(1);         
    RA8875_LCD_DataWrite(0x02);  
    Delay1ms(1);
}
...
void Active_Window(uint16_t XL,uint16_t XR ,uint16_t YT ,uint16_t YB)
{
    ...
    //水平起始位置两字节
	temp=XL;   
    RA8875_LCD_CmdWrite(0x30);//HSAW0
	RA8875_LCD_DataWrite(temp);
	temp=XL>>8;   
    RA8875_LCD_CmdWrite(0x31);//HSAW1	   
	RA8875_LCD_DataWrite(temp);

	//水平结束位置两字节
	temp=XR;   
    RA8875_LCD_CmdWrite(0x34);//HEAW0
	RA8875_LCD_DataWrite(temp);
	temp=XR>>8;   
    RA8875_LCD_CmdWrite(0x35);//HEAW1	   
	RA8875_LCD_DataWrite(temp);

    //setting active window Y
    //垂直起始位置两字节
	temp=YT;   
    RA8875_LCD_CmdWrite(0x32);//VSAW0
	RA8875_LCD_DataWrite(temp);
	temp=YT>>8;   
    RA8875_LCD_CmdWrite(0x33);//VSAW1	   
	RA8875_LCD_DataWrite(temp);

	//垂直结束位置两字节
	temp=YB;   
    RA8875_LCD_CmdWrite(0x36);//VEAW0
	RA8875_LCD_DataWrite(temp);
	temp=YB>>8;   
    RA8875_LCD_CmdWrite(0x37);//VEAW1	   
	RA8875_LCD_DataWrite(temp);
}
...
{ 
	...
	//系统接口方式与颜色模式设定
	RA8875_LCD_CmdWrite(0x10);	 //SYSR   bit[4:3]=00 256 color  bit[2:1]=  00 8bit MPU interface
	RA8875_LCD_DataWrite(0x0C);   //                 1x 64k color             1x	16bit 						 

	//图层模式与扫描方向设定
	RA8875_LCD_DataWrite(0x20);//Vertical Non-Display area = (VNDR + 1) 							  
	RA8875_LCD_CmdWrite(0x04); //VNDR1 //Vertical Non-Display Period Bit [8]  

	//频率周期设定
	 RA8875_LCD_CmdWrite(0x04); 	//PCLK inverse
	 RA8875_LCD_DataWrite(0x01);	//01
	 Delay1ms(1);
	 
	 //Horizontal set
	 RA8875_LCD_CmdWrite(0x14); //HDWR//Horizontal Display Width Setting Bit[6:0]                      
	 RA8875_LCD_DataWrite(0x63);//Horizontal display width(pixels) = (HDWR + 1)*8         	 
	 RA8875_LCD_CmdWrite(0x15);//Horizontal Non-Display Period Fine Tuning Option Register (HNDFTR)   
	 RA8875_LCD_DataWrite(0x03);//Horizontal Non-Display Period Fine Tuning(HNDFT) [3:0]     	 
	 RA8875_LCD_CmdWrite(0x16); //HNDR//Horizontal Non-Display Period Bit[4:0]                         
	 RA8875_LCD_DataWrite(0x03);//Horizontal Non-Display Period (pixels) = (HNDR + 1)*8                
	 RA8875_LCD_CmdWrite(0x17); //HSTR//HSYNC Start Position[4:0]                                      
	 RA8875_LCD_DataWrite(0x02);//HSYNC Start Position(PCLK) = (HSTR + 1)*8                            
	 RA8875_LCD_CmdWrite(0x18); //HPWR//HSYNC Polarity ,The period width of HSYNC.                     
	 RA8875_LCD_DataWrite(0x00);//HSYNC Width [4:0]   HSYNC Pulse width(PCLK) = (HPWR + 1)*8    
	 //Vertical set                             
	 RA8875_LCD_CmdWrite(0x19); //VDHR0 //Vertical Display Height Bit [7:0]                            
	 RA8875_LCD_DataWrite(0xdf);//Vertical pixels = VDHR + 1                                           
	 RA8875_LCD_CmdWrite(0x1a); //VDHR1 //Vertical Display Height Bit [8]                              
	 RA8875_LCD_DataWrite(0x01);//Vertical pixels = VDHR + 1                                           
	 RA8875_LCD_CmdWrite(0x1b); //VNDR0 //Vertical Non-Display Period Bit [7:0]      	 
	 RA8875_LCD_DataWrite(0x14);//Vertical Non-Display area = (VNDR + 1)          
	 RA8875_LCD_CmdWrite(0x1c); //VNDR0 //Vertical Non-Display Period Bit [7:0]      	 
	 RA8875_LCD_DataWrite(0x00);//Vertical Non-Display area = (VNDR + 1)  
	 RA8875_LCD_CmdWrite(0x1d); //VSTR0 //VSYNC Start Position[7:0]                                    
	 RA8875_LCD_DataWrite(0x06);//VSYNC Start Position(PCLK) = (VSTR + 1)                              
	 RA8875_LCD_CmdWrite(0x1e); //VSTR1 //VSYNC Start Position[8]                                      
	 RA8875_LCD_DataWrite(0x00);//VSYNC Start Position(PCLK) = (VSTR + 1)                              
	 RA8875_LCD_CmdWrite(0x1f); //VPWR //VSYNC Polarity ,VSYNC Pulse Width[6:0]                        
	 RA8875_LCD_DataWrite(0x01);//VSYNC Pulse Width(PCLK) = (VPWR + 1)                                 
    
	Active_Window(0,799,0,479);	
}

void RA8875_InitHard__(void)
{
    LCD_Initial();
}
#if 1
...
void RA8875_StartDirectDraw(uint16_t _usX, uint16_t _usY, uint16_t _usHeight, uint16_t _usWidth)
{
	...
	RA8875_WriteCmd(0x02); 		/* 准备读写显存 g_LcdHeight*/
...
//     2. 设定 BTE 宽度缓存器 ? REG[5Ch], [5Dh]
//     3. 设定 BTE 高度缓存器 ? REG[5Eh], [5Fh]
//     4. 设定 BTE 控制缓存器 ? REG[51h] = 01h
...
//     8. 继续步骤 6、 7 直到图像数据全部被读出
    ...
    RA8875_WaitBusy();
	RA8875_REG = 0x54; RA8875_RAM = _usX;
	RA8875_REG = 0x55; RA8875_RAM = _usX >> 8;
	RA8875_REG = 0x56; RA8875_RAM = _usY;
	RA8875_REG = 0x57; RA8875_RAM = _usY >> 8;
    
	RA8875_REG = 0x5C; RA8875_RAM = 0;
	RA8875_REG = 0x5D; RA8875_RAM = 1;
    
	RA8875_REG = 0x5E; RA8875_RAM = 0;
	RA8875_REG = 0x5F; RA8875_RAM = 1;
    
	RA8875_REG = 0x51; RA8875_RAM = 1;
    
	RA8875_REG = 0x50; RA8875_RAM |= (1<<7);
...
void MemoryRead_Cursor_NoautoIncrease(void)
{
	...
	RA8875_LCD_CmdWrite(0x40);//MWCR0
	temp = RA8875_LCD_DataRead();
	temp &= 0xfe;
	RA8875_LCD_DataWrite(temp);
}
...
//     RA8875_REG = 0x40; RA8875_RAM = 1;
//     RA8875_REG = 0x45; temp = RA8875_RAM;
...
// 	RA8875_REG = 0x02;
    ...
    for(j = 0; j < y; j++)
    {
        ...
        {
            RA8875_REG = 0x02;
            buf[i + j * x] = RA8875_RAM;
        }
    }
}
uint16_t RA8875_GetPixelGUI_(uint16_t _usX, uint16_t _usY)
{
	uint16_t usRGB = 0;
