...
COM_STRUCT com4={
    ...
    get_com4_send_frame,///<发送函数
    get_com4_resend_frame,///<重发函数
    com_set_wait_ack_timeout,///<设置等待从机响应的超时时间函数
    com4_wait_ack_timeout,///<等待从机应答超时函数
    receive_over_dispose,///<接收完成后的处理函数
    get_com4_receive_over_flag,///<获取接收完成标志
    get_com4_frame,///<获取串口数据帧
...
/**
  * @brief  第4路串口发送数据函数,直接调用串口驱动层的发送函数
  * @param  [in] com 串口类指针
  * @param  [in] data 数据
  * @param  [in] len 数据长度
  * @retval None
...
/**
  * @brief  第4路串口重发函数,直接调用串口驱动层提供的重发函数
  * @param  [in] com 串口类指针
  * @retval None
  */
static void get_com4_resend_frame(COM_STRUCT *com)
