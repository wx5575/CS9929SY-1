...
#define _GR_RES_H(temp_cur)		((type_spe.gr_cur_h / 100.0) / (temp_cur) * 1000.0 * type_spe.gr_max_cur_res_rang + 0.5)///<计算电阻上限值
#define GR_RES_H(temp_cur)		((_GR_RES_H(temp_cur) > 5100)? 5100:_GR_RES_H(temp_cur))///<计算电阻上限值
...
extern CS_BOOL check_range_validity(uint8_t mode, uint8_t range, uint8_t *gear);
...
extern uint16_t get_defined_g_mode_num(void);
extern void *get_defined_g_mode_flag(void);
extern uint16_t get_defined_g_mode_first_step_num(void);
extern void *get_defined_g_mode_first_step_flag(void);
extern void *get_defined_g_mode_no_first_step_table(void);
